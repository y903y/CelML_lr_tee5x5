...
int main ( int argc , char** argv ) {
	...
	for(membrane_time = 0.000000; ( membrane_time <= 500.000000 ); membrane_time =  ( membrane_time + deltat ) ){
		...
		/*membrane_V__n[]を各ノードに送信(1対全通信)*/
		for(sourcerank = 0; sourcerank < nodenum; sourcerank++){
			if(sourcerank < nodenum - 1) MPI_Bcast(&membrane_V__n[6+nodenum*sourcerank], nodenum, MPI_DOUBLE, sourcerank, MPI_COMM_WORLD);
			else if (sourcerank == nodenum -1) MPI_Bcast(&membrane_V__n[6+nodenum*sourcerank],nodenum+(18-6)%nodenum , MPI_DOUBLE, sourcerank, MPI_COMM_WORLD);
			else return -1;
		}
